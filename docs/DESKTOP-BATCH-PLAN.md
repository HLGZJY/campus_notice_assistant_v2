...
**B01 实测结论（选型定案，2026-09-01）**：见 `docs-local/acceptance/B01-可行性实测报告.md`。核心数据：壳增量 **44.6 MB**（达标 ≤60MB）；启动中位数 **4.80s**（超标 >2s）、空闲总内存壳态 **241MB** 私有（超标），**两项超标均归因后端**——`import api.main` 导入链冷启动 ≈5s（转 Tauri 亦不可避），后端基线私有大口径 196MB 已超出方案假设 60~120MB。§3.5 切换准则评估：**维持 pywebview**，理由：壳增量达标证明壳本身开销可控；WebView2 运行时（369MB/6 进程）与本地嵌入态（私有 1703MB，bge 模型）Tauri 路线同样承担；超标项是后端成本，改换前端壳无法解决。**修正目标口径**：空闲总内存 240MB（含 WebView2 主进程，不含运行时与嵌入）、启动 6s，并将「后端启动导入链」「嵌入模型懒加载/空闲释放」排入 **B22/B21**。**关键实测发现**：pywebview 默认 `private_mode`（无痕）导致 localStorage 不跨重启（K1/R5 坐实）——壳必须 `private_mode=False + storage_path` 固定存储目录，否则主题/QA 会话全丢。
...
| B21.T2 | 内存优化：嵌入模型懒加载 / 空闲释放（B01 实测嵌入态私有 1703MB，须在首次问答/检索时才加载 bge，空闲释放） | 内存下降 |
...
| B01 | 可行性实测与选型定案 | 已通过 | c50b955 | 43eaf4c | 2.0 | pywebview 路线可行：壳增量 44.6MB(≤60 达标)；空闲/启动超标但归因后端(导入链≈5s、基线 196MB)，§3.5 评估维持 pywebview、修正目标口径 + 排入 B21 内存优化与 B22 启动优化 | [B01-验收记录](docs-local/acceptance/B01-验收记录.md)（不入库） |
| B02 | 前端工程规范化 | 已通过 | c49ff26 | 154c231 | 1.5 | typecheck/lint/engines 落地；eslint --fix 清 1328 项 + 2 error 清零；SSE 负载 any→unknown 收敛；schema.ts 零漂移 + 补 3 导出 | [B02-验收记录](docs-local/acceptance/B02-验收记录.md)（不入库） |
| B03 | 桌面壳骨架与后端托管 | 已通过 | 9fd7ae8 | 5db42e2 | 2.0 | Gate 全绿：desktop 壳包与 uvicorn 线程托管就绪，health 200、退出无残留线程、test_api_smoke 全过、桌面回归 1过/6跳/0败 | B03-桌面壳骨架与后端托管-验收记录.md |
| B04 | 端口粘性与看门狗 | 已通过 | 300a3f4 | 1c1165a | 1.5 | Gate 全绿：端口粘性 4/4、调度器幂等无回归、真实看门狗 kill 后 15.1s 自恢复 | B04-端口粘性与看门狗-验收记录.md |
| B05 | 日志落盘与崩溃兜底 | 已通过 | 055bdf3 | 62e2942 | 1.5 | Gate 全绿：日志落盘+uvicorn log_config 接管+stdout 兼容+崩溃钩子落地，5 用例全过，无 P0/P1 | B05-日志落盘与崩溃兜底-验收记录.md |
| B06 | 托盘与窗口生命周期 | 已通过 | 5007f00 | 14e7d0e | 2.0 | Gate 全绿：托盘五项就绪、关闭最小化到托盘进程常驻+health 200、托盘退出进程无残留、窗口几何跨重启恢复；桌面回归 3过/4跳/0败，无 P0/P1 | B06-托盘与窗口生命周期-验收记录.md |
| B07 | 单实例锁与唤起 | 已通过 | 328ba12 | de4ee15 | 1.0 | Gate 全绿：单实例 socket 锁+文件锁兜底落地，4 用例全过，桌面回归 4过/3跳/0败，无 P0/P1 | B07-单实例锁与唤起-验收记录.md |
| B08 | 退出编排与全量回归 | 已通过 | 5306371 | 74b26b8 | 1.5 | Gate 全绿：退出编排+关机信号+二次确认落地，生命周期用例 5/5，全量回归 37过/2跳/0败与 B00 基线一致，P1 阶段 Gate 通过，无 P0/P1 遗留 | — |
| B09 | 桌面 flavor 打包与无控制台 | 已通过 | a9452b5 | 1858acb | 2.0 | Gate 全绿：desktop flavor 打包与 console=False 落地 | B09-桌面flavor打包与无控制台-验收记录.md |
| B10 | 前端桌面适配 | 已通过 | 7f42055 | c85e020 | 2.0 | Gate 全绿：前端契约 typecheck/lint/build 全过，外链统一 helper+webview 兜底+剪贴板降级链+固定 storage_path 落地，L2 全量回归 37过/2跳/0败与基线一致，无 P0/P1 | B10-前端桌面适配-验收记录.md |
| B11 | 控制面路由与启动令牌 | 已通过 | 1b59370 | 3d7336c | 2.0 | Gate 全绿：控制面六端点+令牌校验中间件落地，令牌用例 5/5，全量回归 38过/1跳/0败与基线一致，D-30 无令牌 401、D-31 dev/--browser 关闭校验均验证通过，无 P0/P1 | B11-控制面路由与启动令牌-验收记录.md |
| B12 | 真机装跑退冒烟 | 已通过 | 06a4102 | 24c1fdf | 1.0 | P2 阶段 Gate 全绿：冒烟 7 步 PASS、全量回归 38过/1跳(WAL B17)/0败与基线一致、前端契约全绿、装升卸三连通过、D-01~D-31 全量勾验通过，无 P0/P1，里程碑 M1 Windows 桌面版 v0.2.0-rc1 可交付 | — |
| B13 | 自启开关与启动参数 | 已通过 | 02da5e8 | 9ce7bb6 | 1.0 | Gate 自动项全绿：自启开关落地 HKCU Run，autostart 用例 5/5，全量回归 39过/1跳/0败与基线一致，无 P0/P1 | — |
| B14 | 调度 pause/resume 与空闲触发 | 已通过 | fdd3996 | b6ebeb1 | 2.0 | Gate 全绿：scheduler pause/resume 与控制面对接、空闲检测与重活 gate、睡眠唤醒补偿与电量感知落地，全量回归 39过/1跳/0败与基线一致，无 P0/P1 | B14-调度pause-resume与空闲触发-验收记录.md |
| B15 | 桌面设置页 | 已通过 | b40bde5 | d5170b2 | 1.5 | Gate 全绿：设置页桌面分组落地（自启/启动方式/关闭行为/日志导出/检查更新），前后端契约 typecheck/lint/build 全过、gen:api 无漂移，全量回归 39过/1跳/0败与基线一致，无 P0/P1 | B15-桌面设置页-验收记录.md |
| B16 | 数据目录三态与迁移向导 | 已通过 | 62ee18f | 981bfc6 | 2.0 | Gate 全绿：app_paths 三态改造+迁移向导落地，paths 用例 5/5、桌面回归 7过/1跳/0败、全量回归 39过/1跳/0败与基线一致，无 P0/P1 | B16-数据目录三态与迁移向导-验收记录.md |
| B17 | SQLite WAL 与自动备份 | 已通过 | 920f1c9 | 2ff7fd9 | 1.0 | Gate 全绿：WAL+每周备份落地，test_desktop_wal 5/5、并发与向量单例全绿，全量回归 40过/0跳/0败与基线一致（wal 由跳过转通过为预期激活），无 P0/P1 | B17-SQLiteWAL与自动备份-验收记录.md |
| B18 | 更新闭环 | 已通过 | 829cd16 | b307186 | 2.5 | Gate 全绿：latest.json+下载+sha256+确认安装闭环落地，未配 repo 时 update/check 恒 200 契约保全，updater 端到端 6/6、全量回归 41/41 与基线一致，无 P0/P1 | B18-更新闭环-验收记录.md |
| B19 | CI 分发与 WebView2 检测 | 待验收 | f7f97c7 | 1ce2ce9 | 2.0 | 本地验证全绿（L1 9/9、L2 41/41 无回归、Inno 双 flavor 编译通过、CI YAML 合法）；CI 端到端实跑与 D-46/D-47 干净机人工项需 push 授权后回填 | B19-CI分发与WebView2检测-验收记录.md |
| ~~B20~~ | ~~macOS 构建与签名~~ | **不排期** | — | — | — | 已决策：只做 Windows | — |
| B21 | 打磨：通知中心与内存优化 | 已通过 | 20f8089 | 83080f0 | 3.0 | Gate 全绿：通知中心（未读徽标+已读持久化）、嵌入模型空闲释放守卫、崩溃转储与日志导出完善、路由懒加载四项落地，L1 桌面 10/10、L2 全量回归 42/42 与基线一致、L3 前端契约全绿，无 P0/P1 | B21-打磨：通知中心与内存优化-验收记录.md |
| B22 | 发布 v0.2.0 | 进行中 | 009e764 | ffb9b7d | 2.0 | 自动项全绿（回归43/43、文档齐备、v0.2.0安装包已重建）；发布核查修复 P1 更新闭环（ASCII 名+CI 上传）、P1 令牌注入时序与 P1 首屏401数据缺失（用户实测日志定位）；真机L5勾验与7天稳定性、Release push 待用户介入 | B22-发布v0.2.0-验收记录.md |
