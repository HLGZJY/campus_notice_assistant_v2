...
const sourceOptions = computed(() =>
  (configStore.sources?.sources ?? []).map((s) => ({
    label: s.name,
    value: s.name,
  })),
);
...
function renderSourceLabel(option: SelectOption, selected?: boolean) {
  return h(
    NEllipsis,
    {
      ...
      tooltip: selected ? false : { placement: "top" },
    },
...
const statusOptions = computed(() => {
  ...
  const opts = items
    ...
    .concat(
      FALLBACK_STATUSES.filter((v) => !seen.has(v)).map((v) => ({
        label: v,
        value: v,
      })),
    );
...
const typeOptions = computed(() => {
  ...
  const opts = items
    ...
    .concat(
      FALLBACK_TYPES.filter((v) => !seen.has(v)).map((v) => ({
        label: v,
        value: v,
      })),
    );
...
function rangeToParams(r: [number, number] | null): {
  from?: string;
  to?: string;
} {
  if (!r) return {};
...
async function runCrawl() {
  ...
  try {
    const params: Record<string, unknown> = {
      ...
      deep_check: crawlDeepCheck.value,
    };
    ...
    if (task.status === "success") {
      const summary = task.result?.summary as
        | { new?: number; failed?: number; deep_check?: boolean }
        | undefined;
      message.success(
...
async function openExtractPreview() {
  ...
  try {
    const res = await post<ExtractPreviewResponse>(
      endpoints.notices.extractPreview,
    );
...
async function generateTodos(item: NoticeSummary) {
  ...
  try {
    ...
    const task = await taskStore.submit("generate_todos", {
      notice_id: item.id,
    });
    if (task.status === "success") {
...
function onBatchDelete() {
  dialog.warning({
    title: "批量删除",
    content:
      "确定删除当前筛选条件下的所有通知？删除后不可恢复（含关联待办/提醒）。",
    positiveText: "删除",
    negativeText: "取消",
    onPositiveClick: async () => {
      await runBatch(
...
function onBatchReset() {
  dialog.warning({
    title: "批量重置",
    content:
      "确定重置当前筛选条件下的所有通知？将清空提取结果，状态回到「未提取」。",
    positiveText: "重置",
    negativeText: "取消",
    onPositiveClick: async () => {
      await runBatch(
        async () => {
          const res = await notices.batchReset({
            ...buildFilter(),
            target_status: "raw",
          });
          const task = await poll(res.task_id);
        ...
        <n-space align="center" wrap class="header-actions">
          <div class="action-group">
            ...
            <n-tooltip trigger="hover" placement="top">
              ...
              删除抓取时间 ≤
              {{ oldCutDay }} 的通知（含关联待办/提醒），删除后不可恢复
            </n-tooltip>
          </div>
          <n-button
            size="small"
            ...
            <div class="notice-content">
              <div class="notice-top">
                <n-tag
                  size="small"
                  :bordered="false"
                  :type="statusTagType[item.status] ?? 'default'"
                  round
                >
      ...
      <n-spin :show="previewLoading">
        <n-space vertical size="large">
          ...
          <div v-if="previewSkipped.length">
            <div style="margin-bottom: 8px">
              预筛跳过 {{ previewSkipped.length }} 条（不调 LLM）：
            </div>
          ...
          <div v-if="!previewPassed.length && !previewSkipped.length">
            暂无待提取的通知
          </div>
        </n-space>
      </n-spin>
